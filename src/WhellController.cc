...
public:
  WheelController() : Node("wheel_controller") {
    ...
    RCLCPP_INFO(this->get_logger(), "Wheel controller node started. Aguardando modo terrestre.");
  }
...
private:
  void mode_callback(const std_msgs::msg::Bool::SharedPtr msg) {
    ...
    RCLCPP_INFO(this->get_logger(), "Modo atualizado: %s", ground_mode_ ? "TERRA (rodas ativadas)" : "ÁGUA (rodas desativadas)");
  }

  void joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg) {
    if (!ground_mode_) {
      RCLCPP_DEBUG(this->get_logger(), "Comando ignorado (modo água ativo).");
      return;
    }
    ...
    RCLCPP_DEBUG(this->get_logger(), "Rodando: LF=%.2f, LR=%.2f, RF=%.2f, RR=%.2f",
                 left_front.data, left_rear.data, right_front.data, right_rear.data);
  }
